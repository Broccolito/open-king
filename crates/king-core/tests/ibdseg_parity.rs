...
fn seg_rows_report() {
    ...
    let (mut tot_types, mut tot_dsum) = (0usize, 0.0f64);
    ...
    for ds in DATASETS {
        ...
        for i in 0..fs_.samples.len() {
            for j in i + 1..fs_.samples.len() {
                ...
                got.insert(
                    ...
                    (s.ibd1_seg(denom), s.ibd2_seg(denom), s.prop_ibd(denom)),
                );
            }
        }
        ...
        for (k, w) in &want_rows {
            if let Some(g) = got.get(k) {
                ...
                if ibdseg::inf_type(g.0, g.1, g.2) == ibdseg::inf_type(w.0, w.1, w.2) {
                    types += 1;
                }
        ...
        eprintln!(
            "{ds:<12} gold={:<5} ours={:<5} exact={:<5} infType={:<5} missing={:<4} extra={:<4} maxdPropIBD={:.4}",
            want_rows.len(),
            got.len(),
            exact,
            types,
    ...
    eprintln!(
        "TOTAL gold={tot_rows} exact={tot_exact} infType={tot_types} missing={tot_missing} extra={tot_extra} meandPropIBD={:.4} worst={worst:.4}",
