...
//! source. The module owns three things the readers in `open-king-io` deliberately do not:
//!
...
use open_king_io::{bed, bim, fam, Fileset, IoError, Variant, VariantFilter};
...
pub struct Loaded {
    ...
    pub x_genotypes: Option<open_king_io::Genotypes>,
}
...
///
/// Two length cases the readers in `open-king-io` treat as errors are handled here first,
...
fn decode(
    ...
    variants: &[Variant],
    keep: &[bool],
) -> Result<(open_king_io::Genotypes, Vec<usize>), DecodeError> {
...
/// Encode a keep list as the stand-in `.bim` column `open-king-io` filters on.
///
/// `open-king-io` selects variants through [`VariantFilter`], whose chromosome model stops at
...
/// decided here (see [`classify`]) and `open-king-io` is handed the resolved answer: chromosome
...
/// Malformed-input errors `open-king-io` reports and the reference does not, surfaced as a
