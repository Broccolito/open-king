...
use open_king_io::{Genotypes, Sample, Variant};
...
///
/// `open_king_core::ibdseg` owns the calling rule; this module owns the columns. Keeping the
...
impl PairIbd {
    ...
    /// [`open_king_core::ibdseg::inf_type`] with its `Dup/MZ` clause additionally gated on the
...
impl Engine {
    ...
    /// [`open_king_core::ibdseg::pair_segments`]; `max_ibd2` is the longest IBD2 call of the
