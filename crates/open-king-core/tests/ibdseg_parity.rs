...
//! (`open_king_core::ibdseg::seg_prop_ibd`), while `<prefix>.kin` uses full precision — the
...
use open_king_core::ibdseg;
use open_king_io::{bed, Variant, VariantFilter};
...
fn allsegs_is_byte_identical() {
    ...
    for ds in DATASETS {
        ...
        let Ok(variants) = open_king_io::bim::read_bim(&root.join(format!("{ds}.bim"))) else {
            continue;
        };
