...
//! over it with `--ibs --kinship`, then runs `open-king-io` + `open-king-core` over the same files
...
//!
//! ```text
//! KING_REF=/path/to/king cargo test -p open-king-core --test reference_parity -- --nocapture
//! ```
//!
...
use open_king_core::infer::{KinshipCache, Pedigree};
use open_king_core::{counts, kinship, PairCounts, Scope};
use open_king_io::{bed, Fileset, VariantFilter};
...
impl Engine {
    fn load(prefix: &Path) -> Self {
        let fileset = bed::read_fileset(prefix, VariantFilter::Autosomes, None, None)
            .unwrap_or_else(|e| panic!("open-king-io failed on {}: {e}", prefix.display()));
        let index = fileset
            .samples
            .iter()
            .enumerate()
...
fn check_pedigree(
    ...
    let phi = open_king_core::infer::pedigree_kinship(&engine.pedigree, cache, a, b);
    let z0 = open_king_core::infer::pedigree_z0(&engine.pedigree, cache, a, b);
...
fn parity(king: &Path, ds: &Dataset) -> usize {
    ...
    assert_eq!(
        ...
        expected_autosomes,
        "[{}] open-king-io kept {} markers, the reference kept {}",
        ds.name,
        engine.fileset.kept.len(),
        expected_autosomes
    );
...
fn parity_mixed_chromosome_map() {
    ...
    // and open-king-io's kept set before it compares a single pair.
    assert_eq!(
        ds.markers
            .iter()
            .filter(|m| reference_keeps(&m.chrom))
...
struct Probe {
    ...
    /// Markers `open-king-io` loaded under `VariantFilter::Autosomes`.
    our_kept: usize,
    /// `N_SNP` `open-king-core` computed for the same pair.
    our_n_snp: usize,
}
...
fn own_het_count(g: &open_king_io::Genotypes, s: usize) -> u32 {
    g.plane0[s]
        .iter()
        .zip(&g.plane1[s])
...
fn own_call_count(g: &open_king_io::Genotypes, s: usize) -> u32 {
    g.plane0[s]
        .iter()
        .zip(&g.plane1[s])
        .map(|(&p0, &p1)| (p0 | p1).count_ones())
