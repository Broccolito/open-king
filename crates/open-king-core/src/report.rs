...
use open_king_io::{bed, Fileset, IoError, Sample, VariantFilter};
...
fn validate(fileset: &Fileset) -> Result<(), BundleError> {
    open_king_io::fam::check_duplicates(&fileset.samples)?;
...
#[cfg(test)]
mod tests {
    use open_king_io::{Fileset, Genotypes, Sample, Variant};
